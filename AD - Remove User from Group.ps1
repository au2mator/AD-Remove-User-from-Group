...
# Type: New Service
#
# Title: AD - Remove USer from Group
#
# v 1.0 Initial Release
# v 1.1 Added Stored Credentials
#       see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_RemoveUserFromGroup&utm_content=PS1
...
# Code Template V 1.3
#
# URL:  https://au2mator.com/remove-user-from-group-active-directory-self-service-with-au2mator/?utm_source=github&utm_medium=social&utm_campaign=AD_RemoveUserFromGroup&utm_content=PS1
# Github: https://github.com/au2mator/Remove-User-from-Group
#################
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\AD - Remove User from Group"
[string]$LogfileName = "Remove User from Group"

[string]$CredentialStorePath = "C:\_SCOworkingDir\TFS\PS-Services\CredentialStore" #see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_RemoveUserFromGroup&utm_content=PS1
...
# Stored Credentials
# See: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_RemoveUserFromGroup&utm_content=PS1
...
Write-au2matorLog -Type INFO -Text "Try to remove User from Group"

if (Get-ADGroupMember -Identity $c_Group | Where-Object -Property DistinguishedName -Value $c_User -EQ)
{
    try {
        Remove-ADGroupMember -Identity $c_Group -Members $c_User -Confirm:$false
        Write-au2matorLog -Type INFO -Text "User removed from Group"

        $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) removed from Group $((Get-ADGroup -Identity $c_Group).Name)"
        $AdditionalHTML="<br>
        User $((Get-ADUser -identity $c_User).Name) removed from Group $((Get-ADGroup -Identity $c_Group).Name)
        <br>
        "
        $Status = "COMPLETED"
    }
        ...
        Write-au2matorLog -Type ERROR -Text "Error on removing User from Group"
        Write-au2matorLog -Type ERROR -Text $Error

        $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) failed to removed from Group $((Get-ADGroup -Identity $c_Group).Name), Error: $Error"
        $AdditionalHTML="<br>
        User $((Get-ADUser -identity $c_User).Name) failed to removed from Group $((Get-ADGroup -Identity $c_Group).Name)
        <br>
        Error: $Error
        "
        $Status = "ERROR"
    ...
    Write-au2matorLog -Type INFO -Text "User is not in Group"

    $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) was not a Member of the Group $((Get-ADGroup -Identity $c_Group).Name)"
    $AdditionalHTML="<br>
    User $((Get-ADUser -identity $c_User).Name) was not a Member of the Group $((Get-ADGroup -Identity $c_Group).Name)
    <br>
    "
    $Status = "COMPLETED"
