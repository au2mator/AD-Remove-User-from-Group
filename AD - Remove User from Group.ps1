...
# Code Template V 1.2
# URL: https://au2mator.com/remove-user-from-group-active-directory-self-service-with-au2mator/
...
if (Get-ADGroupMember -Identity $c_Group | Where-Object -Property DistinguishedName -Value $c_User -EQ)
{
    ...
    if ($ErrorCount -eq 0) {
        $au2matorReturn = "User $c_User removed from Group $c_Group"
        $AdditionalHTML="<br>
        User " + (Get-ADUser -identity $c_User).DisplayName +" removed from Group "+(Get-ADGroup -Identity $c_Group).DisplayName+"
        <br>
        "
        $Status = "COMPLETED"
    }
    else {
        $au2matorReturn = "failed to remove $c_User from Group $c_Group, Error: $Error"
        $Status = "ERROR"
    }
}
else {
    ...
    $au2matorReturn = "User $c_User is not in Group $c_Group"
    $AdditionalHTML="<br>
    User " + (Get-ADUser -identity $c_User).DisplayName +" was not a Member of the Group "+(Get-ADGroup -Identity $c_Group).DisplayName+"
    <br>
    "
    $Status = "COMPLETED"
